...
export async function processDataFormula1(season, round, excludedDrivers) {
  ...
  resultsFilters.forEach((result, index) => {
    ...
    result.positionText = result.positionText === 'R' ? result.positionText : String(position)

    if (result.FastestLap && parseInt(result.FastestLap.rank) < fastestLapRank) {
      fastestLap = result
...
export function showDataRaceOnHTML(data) {
  ...
  const headers = ['POS', 'NO', 'DRIVER', 'TEAM', 'LAPS', 'INTERVAL', 'FASTEST LAP', 'PTS']
