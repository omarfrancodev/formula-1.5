export function updateStandings(data, driverStandings, constructorsStandings) {
  data.raceStandings.forEach((result) => {
    const existingDriver = driverStandings.find((d) => d.Driver.driverId === result.Driver.driverId)
    if (existingDriver) {
...
export async function showStandingsOnHTML(driverStandings, constructorsStandings) {
